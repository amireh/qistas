...
class UserService < Service
  ...
  def find_or_create_from_oauth(provider, auth, current_user=nil)
    ...
    if new_user && current_user.nil?
      ...
      master_creation = create(user_info.merge({ provider: User::PROVIDER }))
  ...
  def find_master_user_for(email)
    User.find_by({ email: email, provider: User::PROVIDER })
  end
  ...
  def find_or_create_master_for(user_info)
    ...
    if master = find_master_user_for(user_info[:email])
      ...
      creation_svc = create(user_info.merge({ provider: User::PROVIDER }))
  ...
  def generate_random_password
    str = (Base64.urlsafe_encode64 Random.rand(1234 * (10**3)).to_s(8)).to_s
    Algol::Util.sanitize_string(str)
  end
end