define([ 'rsvp', 'modules/form_error' ], function(RSVP, FormError) {
   ...
   * done using the React.addons.ActorMixin mixin.
   */
  return {
    ...
    componentWillUpdate: function(nextProps, nextState) {
      var storeError = nextProps.storeError || nextState.storeError;

      if (storeError && storeError.actionIndex === nextState.actionIndex) {
    ...
    showFormError: function(apiError) {
      ...
      return this.clearFormError().then(function() {
        ...
        if (this.formError.isEmpty()) {
          if (this.renderFormError) {
            this.renderFormError(apiError);
          }
        }
